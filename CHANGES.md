...
The docs follow upstream's split. README keeps install, Herdr setup, and one table of who owns what, and moves the rest to `docs/reference.md` and HERDR.md. HERDR.md is now the runtime contract only: roles, routing, depth, the dispatch lifecycle with `--collect`, and the Agent gate. `docs/reference.md` installs from this fork. CONTEXT.md defines Herdr route, profile, dispatcher, and handoff. SECURITY.md sends Herdr-code reports to this fork.
...
The dispatcher no longer passes a config home equal to the CLI's default (`CLAUDE_CONFIG_DIR=~/.claude`, `CODEX_HOME=~/.codex`) to the worker, unless the shell that started Herdr exports a different home for that variable. Claude Code moves its onboarding state to `$CLAUDE_CONFIG_DIR/.claude.json` once the variable is set, so a worker given the default home booted into the first-run theme picker and `agent prompt` failed with `agent_prompt_stalled`. `config/routes.example.yaml`, `config/setup.example.json`, and `/setup-pstack` no longer suggest the default home; the routes writer still records whatever `config_home` it is given.

Prompt delivery is now checked rather than assumed. A prompt typed into a Codex CLI that was still starting was lost: Herdr read the cold start as `working`, the startup turn's completion satisfied `agent prompt --wait`, and the dispatcher reported `done` over an empty composer with no session started. The dispatcher now waits for the worker to report idle before typing, counts the prompt delivered once the agent leaves idle within `PSTACK_HERDR_DELIVERY_WINDOW_MS` (default 8000 ms), retries once, and only then runs `agent wait`. A prompt that is not accepted or not delivered closes the pane and puts the worker's last visible screen in the error, with a long prompt shown by size rather than text.
