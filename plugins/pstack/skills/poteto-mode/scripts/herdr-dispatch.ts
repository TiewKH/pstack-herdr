...
export function selectProfileName(config: RoutesConfig, role: string, workerName: string): string | undefined {
  ...
  if (strategy === "round-robin" || strategy === "spread") {
...
function paneId(payload: unknown): string {
  const value = payload as { result?: { pane?: { pane_id?: string } } };
...
function parentKind(): AgentKind | undefined {
  const value = process.env.PSTACK_HERDR_PARENT_KIND;
...
function chooseProfile(config: RoutesConfig, options: DispatchOptions): { name: string; profile: Profile } {
  ...
  const kind = options.kind ?? parentKind();
  if (!kind) {
    throw new Error("no route matched; set --kind claude|codex or PSTACK_HERDR_PARENT_KIND");
  }
  return {
    name: "parent",
...
export async function dispatch(options: DispatchOptions): Promise<unknown> {
  ...
  if (!/^[a-z][a-z0-9_-]{0,31}$/.test(options.name)) {
  ...
  const splitArgs = [
    "pane", "split", "--current", "--direction", options.direction,
    "--cwd", resolve(options.cwd), "--no-focus",
  ];
  const env = { ...(chosen.profile.env ?? {}), PSTACK_HERDR_DEPTH: String(nesting.next) };
  for (const [key, value] of Object.entries(env)) {
    splitArgs.push("--env", `${key}=${expandHome(value)}`);
  }
  ...
  if (!options.wait) {
    return { agent: options.name, pane, profile: chosen.name, kind: chosen.profile.kind, depth: nesting.next };
  }
