...
| `no-comments` | Use `--role reviewer --readonly`. Build the worker brief from the shipped `agents/comment-sicko.md` contract plus the requested scope; Herdr has no Claude `subagent_type`, so do not silently drop that reviewer contract. |
