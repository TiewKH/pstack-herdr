...
The playbooks keep a todolist using the session's task-tracking tools, with an uncommitted `todo.md` fallback. Claude Code's defaults depend on the model; set `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` to enable the tools. Scope it to one project through the `env` block of that project's `.claude/settings.local.json` when Claude also drives a personal task system. See the [task-tool documentation](https://code.claude.com/docs/en/tools-reference#task-tool-availability) and [poteto-mode's checklist policy](plugins/pstack/skills/poteto-mode/SKILL.md#platform-adaptation).
...
- **Commands.** The 31 `.codex-plugin/prompts/*.md` files are Codex-only, generated by `tools/generate.mjs` from each public skill's row in this README's slash-command table (edit the row, rerun the generator; hand edits to a stub are overwritten). Codex reads their `description` frontmatter and the filename, ignores the keys it doesn't know, and each body invokes its skill. Link them into `~/.codex/prompts/` for `/name` shortcuts (see [Install on Codex](#codex)). Claude Code ships no `commands/` directory: it renders both commands and user-invocable skills in the slash menu, so a trampoline paired with its skill duplicated every `/pstack:<name>` row (see CHANGES 0.9.13). The skill alone serves the slash command there.
- **Tool, model, and skill mapping.** When a skill names a Claude tool (the `Agent` tool, `AskUserQuestion`), a `claude-*` model slug, or a driver or bundled skill, it resolves through [`skills/poteto-mode/references/codex-tools.md`](plugins/pstack/skills/poteto-mode/references/codex-tools.md). `poteto-mode`'s Platform Adaptation section, affected skill entry points, and every generated Codex stub point there; skill-specific mappings sit in its Per-skill notes table.
...
- **`run`, `loop`** are bundled Claude Code skills. App verification uses the [driver policy](plugins/pstack/skills/poteto-mode/SKILL.md#non-negotiables). `/create-verification-skill` records a project driver at `.claude/skills/verify/`; [Claude Code 2.1.200 or later](https://code.claude.com/docs/en/skills#run-and-verify-your-app) lets it replace the bundled `/verify`.
...
| `cursor-team-kit` `control-ui` (browser/Electron driver) | The [driver policy](plugins/pstack/skills/poteto-mode/SKILL.md#non-negotiables) |
...
- **`cursor-team-kit` beyond the seven imported skills** covers app verification (`verify-this`, `control-cli`, `control-ui`), handled by the [driver policy](plugins/pstack/skills/poteto-mode/SKILL.md#non-negotiables); compiler diagnostics (`check-compiler-errors`), handled by LSP; or workflows already covered by `babysit`, `fix-ci`, `make-pr-easy-to-review`, and `what-did-i-get-done`. `pr-review-canvas` is Cursor-UI-specific.
